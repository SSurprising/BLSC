...
class ResUNet_2(nn.Module):
    ...
    def __init__(self, training, inchannel, stage):
        ...
        self.encoder_stage1 = nn.Sequential(
            nn.Conv3d(inchannel + 16 + 16, 16, 3, 1, padding=1),
            nn.PReLU(16),
        )
        ...
        self.decoder_stage1 = nn.Sequential(
            nn.Conv3d(128 + 128 + 128 + 128, 128, 3, 1, padding=1),
            nn.PReLU(128),
        ...
        self.decoder_stage2 = nn.Sequential(
            nn.Conv3d(64 + 64 + 64 + 64, 64, 3, 1, padding=1),
            nn.PReLU(64),
        ...
        self.decoder_stage3 = nn.Sequential(
            nn.Conv3d(32 + 32 + 32 + 32, 32, 3, 1, padding=1),
            nn.PReLU(32),
        ...
        self.decoder_stage4 = nn.Sequential(
            nn.Conv3d(16 + 16 + 16 + 16, 16, 3, 1, padding=1),
            nn.PReLU(16),
        )
    ...
    def forward(self, inputs, long_ranges=None, outputs_ranges=None):
        long_range1 = self.encoder_stage1(torch.cat([inputs, long_ranges[0], outputs_ranges[0]], dim=1))
        short_range1 = self.down_conv1(long_range1)
 
        long_range2 = self.encoder_stage2(torch.cat([short_range1, long_ranges[1], outputs_ranges[1]], dim=1)) + short_range1
        long_range2 = F.dropout(long_range2, dropout_rate, self.training)
        short_range2 = self.down_conv2(long_range2)
 
        long_range3 = self.encoder_stage3(torch.cat([short_range2, long_ranges[2], outputs_ranges[2]], dim=1)) + short_range2
        long_range3 = F.dropout(long_range3, dropout_rate, self.training)
        short_range3 = self.down_conv3(long_range3)
 
        long_range4 = self.encoder_stage4(torch.cat([short_range3, long_ranges[3], outputs_ranges[3]], dim=1)) + short_range3
        long_range4 = F.dropout(long_range4, dropout_rate, self.training)
        short_range4 = self.down_conv4(long_range4)
...
#        print('-------decoder-------')
        short_range5 = self.up_conv1(outputs)
        outputs = self.decoder_stage1(torch.cat([short_range5, long_range4, long_ranges[3], outputs_ranges[3]], dim=1)) + short_range5
        outputs = F.dropout(outputs, dropout_rate, self.training)
 
        short_range6 = self.up_conv2(outputs)
        outputs = self.decoder_stage2(torch.cat([short_range6, long_range3, long_ranges[2], outputs_ranges[2]], dim=1)) + short_range6
        outputs = F.dropout(outputs, dropout_rate, self.training)
 
        short_range7 = self.up_conv3(outputs)
        outputs = self.decoder_stage3(torch.cat([short_range7, long_range2, long_ranges[1], outputs_ranges[1]], dim=1)) + short_range7
        outputs = F.dropout(outputs, dropout_rate, self.training)
 
        short_range8 = self.up_conv4(outputs)
        # print('self.up_conv4 = ', short_range8.shape)
 
        outputs = self.decoder_stage4(torch.cat([short_range8, long_range1, long_ranges[0], outputs_ranges[0]], dim=1)) + short_range8
